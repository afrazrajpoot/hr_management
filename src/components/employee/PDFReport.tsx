...
export default function PDFReport({ assessment }: any) {
  ...
  const generatePDF = () => {
    ...
    if (isFieldValid(geniusProfile)) {
      ...
      if (isFieldValid(secondaryGenius)) {
        ...
        if (isFieldValid(secondaryDescription)) {
          addTextContent(secondaryDescription);
        }
      }
    ...
    if (isFieldValid(methodology)) {
      ...
      const dataSources = safeGet(methodology, 'data_sources');

      if (isFieldValid(methodologyText) || isFieldValid(dataSources)) {
        ...
        if (isFieldValid(dataSources)) {
          addSectionHeader("Data Sources");
          addBulletList(dataSources);
        }
      }
    }
